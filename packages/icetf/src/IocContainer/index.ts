...
class IocContainer {
    ...
    registerSingle(keyType: any, valueBuilder: ()=>any){
        ...
        this.singles.set(keyType.iocKey, valueBuilder);
    }
    ...
    registerSingleIntances(keyType: any, value: any){
        ...
        this.singles.set(keyType.iocKey, value);
    }
    ...
    getService(keyType: any){
        let intance = this.intances.get(keyType.iocKey);
        if(intance){
            return intance;
        }

        let singleBuilder = this.singles.get(keyType.iocKey);
        if(singleBuilder){
            this.intances.set(keyType.iocKey, singleBuilder());
            return this.intances.get(keyType.iocKey);
        }

        let transientBuilder = this.transients.get(keyType.iocKey);
        if(transientBuilder){
